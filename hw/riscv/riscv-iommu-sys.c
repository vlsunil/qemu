...
static void riscv_iommu_sysdev_init_msi(RISCVIOMMUStateSys *s, uint32_t n_vectors)
{
    ...
    memory_region_init_io(&s->msix_table_mmio, OBJECT(s), &msix_table_mmio_ops, s,
                          "msix-table", table_size);
    memory_region_add_subregion(&iommu->regs_mr, table_offset, &s->msix_table_mmio);

    memory_region_init_io(&s->msix_pba_mmio, OBJECT(s), &msix_pba_mmio_ops, s,
                          "msix-pba", pba_size);
    memory_region_add_subregion(&iommu->regs_mr, pba_offset, &s->msix_pba_mmio);
}
